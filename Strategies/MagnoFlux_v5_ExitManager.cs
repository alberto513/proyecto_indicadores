...
{
    public class MagnoFlux_v5_ExitManager : Strategy
    {
        #region Parameters
        [NinjaScriptProperty]
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.DataLoaded)
            {
                ...
                if (logWriter != null && logWriter.BaseStream.Length == 0)
                    logWriter.WriteLine("Time,Direction,Entry,Exit,Type,PNL,Score,Hz,EquityPeak,EquityDrawdown");
            }
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (pendingLong || pendingShort)
            {
                if (CurrentBar == signalBar + 1 && IsFirstTickOfBar)
                {
                    if (pendingLong)
                    {
                        Draw.VerticalLine(this, "signal" + CurrentBar.ToString(), 0, Brushes.Lime);
                        Print($"LONG={Time[0]:HH:mm:ss} - Entrada={Open[0]:F2} - SeñalBar={CurrentBar}");
                    }
                    if (pendingShort)
                    {
                        Draw.VerticalLine(this, "signal" + CurrentBar.ToString(), 0, Brushes.Red);
                        Print($"SHORT={Time[0]:HH:mm:ss} - Entrada={Open[0]:F2} - SeñalBar={CurrentBar}");
                    }
        ...
        {
            if (prevPos == MarketPosition.Flat && Position.MarketPosition != MarketPosition.Flat)
            {
                ...
                {
                    tpPrice = isLong ? High[1] : Low[1];
                    slPrice = isLong ? Low[1] : High[1];
                }
                ...
                entryExecTime = Time[0];
            }
            else if (Position.MarketPosition == MarketPosition.Flat && prevPos != MarketPosition.Flat)
            {
                entryBar = -1;
            }
        ...
        protected override void OnExecutionUpdate(Execution execution, string executionId, double price, int quantity, MarketPosition marketPosition, string orderId, DateTime time)
        {
            ...
            if (prevPos == MarketPosition.Flat && marketPosition != MarketPosition.Flat)
            {
                entryExecTime = time;
            }
            else if (marketPosition == MarketPosition.Flat && prevPos != MarketPosition.Flat)
            {
            ...
            {
                tradePnl += tradeDirection * (price - entryPrice) * quantity * (Instrument.MasterInstrument.PointValue == 0 ? 1 : Instrument.MasterInstrument.PointValue);
                if (execution.Order.Name == "TP1" || execution.Order.Name == "TP2" || execution.Order.Name == "TP2Early")
                {
                    double acc = 0; try { acc = Account.Get(AccountItem.CashValue, Currency.UsDollar); } catch { }
                    if (acc > equityPeak) equityPeak = acc;
                    equityDrawdown = equityPeak - acc;
